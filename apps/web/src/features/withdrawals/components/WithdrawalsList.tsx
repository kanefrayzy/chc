...
export function WithdrawalsList({ locale, pageSize = 10 }: WithdrawalsListProps): JSX.Element {
  const t = useTranslations('withdraw.list');
  ...
  const loadPage = (cursor?: string): void => {
    startTransition(async () => {
      try {
        const page = await withdrawalsApi.list({ limit: pageSize, ...(cursor ? { cursor } : {}) });
  ...
  return (
    <div className="space-y-3">
      {items.map((w) => (
        <WithdrawalCard key={w.id} withdrawal={w} locale={locale} />
      ))}
      {nextCursor ? (
